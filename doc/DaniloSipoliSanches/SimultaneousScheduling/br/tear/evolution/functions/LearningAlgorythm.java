...
public class LearningAlgorythm {
	...
	public void learn() {
		...
		while ((i < maxIteration) && (convergence != 0.0)) {// (filter != 1000))
			...
			if (makeReproduction) {
				 ...
				 * tempPopulation);
				 */
				tempPopulation = (Population) reproductionFunction.getNewPopulation().clone();
				saveElitism = elitism.getTempCromo();
				/*
				 * System.out.println("\ntempPopulation pos torneio: \n" +
				 * tempPopulation);
				 * System.out.println("Cromosomome salvo pelo elitismo pos	roleta: "
				 * + saveElitism + " Elistimo da Classe: " +
			 ...
			 */
			if (oldCrossOver != 0.0) {
				if (verifyAboveRate(bestFitnessValue, oldCrossOver, tempPopulation.getMediaMakespan())) {
					if (operatorsRate.getCrossOverRate() > 0.5)
						operatorsRate.setCrossOverRate((operatorsRate.getCrossOverRate()) - 0.05);
					// System.out.println("Cruzamento: "+operatorsRate.getCrossOverRate());
				} else if (verifyBelowRate(bestFitnessValue, oldCrossOver, tempPopulation.getMediaMakespan())) {
					if ((operatorsRate.getCrossOverRate() > 0.5) && (operatorsRate.getCrossOverRate() < 1.0))
						operatorsRate.setCrossOverRate((operatorsRate.getCrossOverRate()) + 0.05);
					// System.out.println("Cruzamento: "+operatorsRate.getCrossOverRate());
				}
			}
			...
			if (oldMutation != 0.0) {
				if (verifyAboveRate(bestFitnessValue, oldMutation,
						tempPopulation.getMediaMakespan())) {
				...
				} else if (verifyBelowRate(bestFitnessValue, oldMutation,
						tempPopulation.getMediaMakespan())) {
					if ((operatorsRate.getMutationRate() >= 0.005)
			...
			// bestChromosome.add((Object)
			// (tempPopulation.getBestChromosome()));
			newPopulation = (Population) tempPopulation.clone();

			/*
			 * System.out.println("Elitismo: " +
			 * newPopulation.getChromosome(newPopulation.size()-1) +
			 * " Makespan : " +
			...
			makespan.setTempSolution(bestChromosome, bestFitnessValue,
					newPopulation.getMediaMakespan(), i);
		 ...
		 * i); } else { System.out.println("Convergiu a partir da itera��o: " +
		 * i); }
		 */

		if (convergence != 0.0) {
			System.out.println("Fim da Itera��o e n�o convergiu: " + i);
		} else {
	 ...
	 */
	public boolean verifyAboveRate(int bestFitness, double oldFitness,
			double popAverage) {
		/*
		 * System.out.println("Atual: "+(popAverage -
		 * bestFitness)+" Antigo: "+oldFitness); System.out.println(popAverage -
		 * bestFitness+" = "+((oldFitness) + oldFitness * 0.1));
		 */
	 ...
	 */
	public boolean verifyBelowRate(int bestFitness, double oldFitness,
			double popAverage) {
		/*
		 * System.out.println("Atual: "+(popAverage -
		 * bestFitness)+" Antigo: "+oldFitness); System.out.println(popAverage -
