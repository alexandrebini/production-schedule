...
public class DefaultMutationFunction extends AbstractMutationFunction {
	...
	public void generateNewPopulation() {
		if (oldPopulation != null) {
			...
			subProgram = oldPopulation.getChromosomeSize() / produto.getQuantidade();
			random = new Random();
			...
			String stringSelectChromosome = String.valueOf(
					Math.round((oldPopulation.size() * operatorsRate.getMutationRate())));
			...
			for (int i = 0; i < chromosomeGroup.size(); i++) {
				...
				auxCopyChromosome.addAll((Vector<Integer>) newPopulation.getChromosome(chromosomeGroup.get(i)));
				...
				if (mutationPoint == 0) {
					while (mutationPoint == 0) {
						mutationPoint = random.nextInt(produto.getQuantidade() + 1);
					}
				}
				...
				for (int point = 1; point <= mutationPoint; point++) {
					if (point == mutationPoint) {
						...
						for (int qtder = 1; qtder <= produto.getRoteiro(subListaC1.get(0)); qtder++) {
							auxConvert.addAll(
									roteiro.getRoute(Integer.parseInt(String.valueOf(subListaC1.get(0)) + String.valueOf(qtder))));
							if (auxConvert.equals(tempSubList)) {
								indexC2 = Integer.parseInt(String.valueOf(subListaC1.get(0)) + String.valueOf(qtder));
							}
							auxConvert.clear();
						}
						...
						indexC3 = Integer.parseInt(String.valueOf(verificaProd.charAt(0)));
						indexC4 = Integer.parseInt(String.valueOf(verificaProd.charAt(1)));
						...
						int mutation = Integer.parseInt(String.valueOf(indexC3) + String.valueOf(verifica));
				...
				System.out.println("Muata��o: " + " Posi��o: " + i + " "
						+ chromosome);*/

				newPopulation.replaceChromosome(chromosomeGroup.get(i), chromosome.clone());
